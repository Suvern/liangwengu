...
module PricingSchema =
    ...
    let parseHHmm (s: string) : int =
        let parts = s.Split(':')
        if parts.Length <> 2 then failwith $"invalid HH:mm: %s{s}"
    ...
    let parse (json: string) : Result<PricingSnapshot, string> =
        try
            ...
            if ver > MAX_SUPPORTED_SCHEMA_VERSION then
                ...
                if snap.Models.IsEmpty then Error "models must be non-empty"
                elif snap.PeakPolicy.Windows.IsEmpty then Error "windows must be non-empty"
                else Ok snap
        with ex ->
            Error ex.Message

    let tryParse (json: string) : PricingSnapshot option =
        match parse json with Ok s -> Some s | Error _ -> None
